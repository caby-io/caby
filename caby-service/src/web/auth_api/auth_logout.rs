...
pub async fn handle_logout(State(_cfg): State<Config>, authorized_user: AuthUser) -> Response {
    let Some(account) = authorized_user.as_account() else {
    ...
    let session_path = account
        .path
        .join(format!("session_{}", authorized_user.token));
